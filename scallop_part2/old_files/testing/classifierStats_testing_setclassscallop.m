...
% windowStat = histc(fixationResults.windowVect, 1:params.fixationWindowSize(:)/2);
% cumVal = (cumsum(windowStat)./sum( [statData.groundTruth.numScallops])).*100;
...
numGroundTruthScallops = sum( [statData.groundTruth.numScallops] )
numSegmentObjects = getTotalObjects(segmentData.reducedCircList)
numBGObjects = length(nonScallopDistrMatchCounts)
...
scallopsBGMatchLost = statData.scallopBGDistrMatchNum
percentScallopsBGMatchLost = statData.scallopBGDistrMatchNum/numGroundTruthScallops
...
function totalObjects = getTotalObjects(reducedCircList)
    totalObjects = 0;
    for i=1:length(reducedCircList)
        totalObjects = totalObjects+length(reducedCircList(i));
    end
end
