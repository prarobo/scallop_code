function [ currDataPt, currDataPtOrig ] = bin2DScallop_linear_testing_mutinfo( img, centerX, centerY, radius, params )
...
currDataPtOrig = zeros(params.resizeImageSize);
...
if xDiff ~= 0 || yDiff ~= 0
    ...
    error('Object too close to boundary! This could lead to serious compatibility issues in other functions, error in function %s', mfilename);
    % return;
end
...
resizeImgOrig = imresize( subImgOrig, [params.resizeImageSize params.resizeImageSize] );
...
for intervalI = 1:length( intervalVect )
    if intervalI == 1
        ...
        currDataPtOrig( resizeImgOrig <= intervalVect(intervalI) ) = intervalI;
    else
        currDataPt( (resizeImg > intervalVect(intervalI-1) ) & (resizeImg <= intervalVect(intervalI) ) ) = intervalI;
        currDataPtOrig( (resizeImgOrig > intervalVect(intervalI-1) ) & (resizeImgOrig <= intervalVect(intervalI) ) ) = intervalI;
    end
end
