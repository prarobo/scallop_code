function [ quadrantData, params ] = quadrant_learning_lookup_eff( params, argNumWidthQuadrants, argNumHeightQuadrants )
...
quadrantData.loc = single(scallopInfoData.loc);
...
quadrants = zeros(params.numQuadrants, 4, 'uint16');
...
scallopQuadrant = zeros(numScallops,1,'uint16');
...
for quadrantI = 1:numQuadrants
        ...
        if quadrantIDTemp(quadrantI).num ~= 0
            quadrantID(quadrantI).meanScallop = single( transpose( reshape( currMeanMat, numResizeImageSize, numResizeImageSize ) ));
            quadrantID(quadrantI).stddevScallop = single( transpose( reshape( currStddevMat, numResizeImageSize, numResizeImageSize ) ));
        else
            quadrantID(quadrantI).meanScallop = single(0);
            quadrantID(quadrantI).stddevScallop = single(0);
        end
        
        quadrantID(quadrantI).numScallops = uint16( quadrantIDTemp(quadrantI).num);
        quadrantID(quadrantI).attributeData = single( attributeData( scallopQuadrant == quadrantI, : ) );
        quadrantID(quadrantI).scallopID = uint16( find( scallopQuadrant == quadrantI ));
