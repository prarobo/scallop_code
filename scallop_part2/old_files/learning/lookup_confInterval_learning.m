function [ confIntervalData, scallopLookupParams ] = lookup_confInterval_learning( params, scallopLookup )
...
scallopLookupParams.confIntervalScallop = params.confIntervalScallop;
scallopLookupParams.confIntervalHalfWindow = params.confIntervalHalfWindow;
...
for rowI = 1:numRows
    ...
    for colI = 1:numCols
        ...
        [isValidConf(rowI,colI), confInterval{rowI,colI}, meanPointsConf{rowI, colI}, stddevPointsConf{rowI, colI}] ...
...
function [isValid, confInterval, meanPoints, stddevPoints] = computeConfInterval(dataPoints, numPoints, numDiscretizationBins, confIntervalScallop, numFeatures )
...
if numPoints
    ...
    if numPoints == 1
        meanPoints = dataPoints;
        stddevPoints = zeros(1,numFeatures);
    else
        meanPoints = mean(dataPoints);
        stddevPoints = std(dataPoints);
    end
