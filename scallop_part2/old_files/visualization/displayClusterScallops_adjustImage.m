function displayClusterScallops_adjustImage( params, clusterData, scallopInfo, fileInfo, annulusOn )
...
for clusterI = 1:numClusters
    if currClusterIndex( clusterI ) ~= prevClusterIndex( clusterI )
        ...
        if annulusOn
            ...
            adjustImage = applyAnnulus( adjustImage, params, ...
                scallopInfo.loc( scallopCluster{clusterI}(currClusterIndex(clusterI)),:), ...
                scallopInfo.diff( scallopCluster{clusterI}(currClusterIndex(clusterI)),:) );
        end

        subplot(2, numClusters, clusterI);        
        imshow(cropImage);

        subplot(2, numClusters, numClusters+clusterI);        
        imshow(adjustImage);
    end
end
...
for layerI = 1:numLayers
    outImage(:,:,layerI) = outImage(:,:,layerI) .* uint8(mask);
end
