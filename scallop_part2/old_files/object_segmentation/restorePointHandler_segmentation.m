function [ imageStatusComplete, reducedCircList ] = restorePointHandler_segmentation( params, imageStatusComplete, reducedCircList )
%RESTOREPOINTHANDLER_SEGMENTATION Restores segmentation points
    ...
    for imageI = newStartImage:newEndImage
        ...
        if( ~imageStatusComplete(imageI) )
            if ( newRestoreMat.imageStatusComplete(imageIndex) )
                reducedCircList(imageI).fixation = newRestoreMat.reducedCircList(imageIndex).fixation;
            end
        end
    end
...
restoreMat.reducedCircList = reducedCircList;
