function [ segmentData, graphData ] ...
    = segmentObjects_errorInj_restoreSwitch( fixationData, groundTruth, params, fileInfo, varargin )
...
% Sanity checks between switches
...
if restoreOn && storeProcessData 
    error('Restore and storing process data cannot be done at the same time, error in function %s', mfilename);
end    
...
if restoreOn
    [imageStatusComplete, reducedCircList] = restorePointHandler_segmentation( params, imageStatusComplete, reducedCircList );
end
...
for imageI = 1:numImages
    ...
    if( ~imageStatusComplete(imageI) )
        ...
        fprintf('Segmenting image %d of %d (left %d images)...',imageI, numImages, numImages-sum(imageStatusComplete) );
        ...
        imageStatusComplete(imageI) = true;
        fprintf('done\n');
    end
end
