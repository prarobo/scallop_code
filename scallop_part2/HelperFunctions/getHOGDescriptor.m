...
% Customized to be used on 24 X 24 windows of scallops.
% It takes images of any size and only checks if the
% dimensions are multiples of 8
% It also pad the image with a border of 1 pixel
...
% Verify if image dimensions are multiples of cellsize
[height, width] = size(img); 
if ((mod(width-2,cellSize) ~= 0) || (mod(height-2, cellSize) ~= 0))
    error('Image dimensions should be multiples of %d\n, error in function %s', cellSize, mfilename);
end
...
numHorizCells = (width-2)/cellSize;
numVertCells = (height-2)/cellSize;
