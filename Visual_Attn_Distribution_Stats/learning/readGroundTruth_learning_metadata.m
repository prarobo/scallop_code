function [ groundTruth, radiusData ] = readGroundTruth_learning_metadata( params, fileInfo, statFile )
...
for imageI = 1:params.numImages
    ...
    for i = 1:numScallops
        groundTruth(imageI).loc(i,1) = scallopData.X(ind(i));
        groundTruth(imageI).loc(i,2) = scallopData.Y(ind(i));
        groundTruth(imageI).loc(i,3) = scallopData.radius(ind(i));
    end
end

radiusData.points = scallopData.origRadius;

%% Compute scallop radius distribution

radiusData.distrFit = fitdist( radiusData.points, 'Normal');
