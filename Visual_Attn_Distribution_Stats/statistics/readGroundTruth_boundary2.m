function [ params, groundTruth ] = readGroundTruth_boundary2( params, fileInfo, statFile )
...
currImageIndex = cellfun(@stripNumbers, fileInfo.filename);
groundTruth(params.numImages) = struct;

for imageI = 1:params.numImages
    ind = find( scallopData.ImageIndex == currImageIndex(imageI) );
    numScallops = length(ind);
    groundTruth(imageI).numScallops = numScallops;
    groundTruth(imageI).numBoundaryScallops = 0;
    groundTruth(imageI).loc = zeros( numScallops, 3 );
    groundTruth(imageI).origRadius = zeros( numScallops, 1);
    groundTruth(imageI).boundaryScallop = false( numScallops, 1);
    
    for scallopI = 1:numScallops
        groundTruth(imageI).loc(scallopI,1) = scallopData.X(ind(scallopI));
        groundTruth(imageI).loc(scallopI,2) = scallopData.Y(ind(scallopI));
        groundTruth(imageI).loc(scallopI,3) = scallopData.radius(ind(scallopI));
        groundTruth(imageI).origRadius(scallopI) = scallopData.origRadius(ind(scallopI));
        ...
        if (currScallopLoc(1) <= round(params.boundaryCropPercent*params.imageSize(2)) || ...
            ...
            currScallopLoc(2) >= params.imageSize(1) - round(params.boundaryCropPercent*params.imageSize(1)))           
                groundTruth(imageI).boundaryScallop(scallopI) = true;
                groundTruth(imageI).numBoundaryScallops ...
                    = groundTruth(imageI).numBoundaryScallops + 1;
        end
    end
end
