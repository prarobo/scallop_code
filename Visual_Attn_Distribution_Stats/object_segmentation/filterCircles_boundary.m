function reducedCircleList = filterCircles_boundary( circList, fixationWindowSize, varargin )
...
%% Clearing Boundary Objects

for circI = 1:numCircles
    if(circleAccept(circI))
        if(     circList(circI,1) <= round(params.boundaryCropPercent*params.imageSize(2)) ||...
                circList(circI,1) >= params.imageSize(2) - round(params.boundaryCropPercent*params.imageSize(2)) ||...
                circList(circI,2) <= round(params.boundaryCropPercent*params.imageSize(1)) ||...
                circList(circI,2) >= params.imageSize(1) - round(params.boundaryCropPercent*params.imageSize(1)) )
            circleAccept(circI) = false;
        end
    end
end
