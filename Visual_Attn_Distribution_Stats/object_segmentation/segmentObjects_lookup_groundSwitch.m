function [ segmentData, graphData ] ...
    = segmentObjects_lookup_groundSwitch( fixationData, groundTruth, params, fileInfo, varargin )
...
for imageI = 1:numImages
    ...
    elseif strcmp(segmentType, 'ground')
        [ currImage, currNumSegments] = createGroundScallopImage( groundTruth(imageI), imageSize );
    else
    ...
    if storeProcessData
        ...
        filterData(imageI).fixation = labelCirc;
    end 
end
...
function [ currImage, currNumSegments] = createGroundScallopImage( groundTruthImage, imageSize )
...
for scallopI = 1:groundTruthImage.numScallops
    ...
    circlesWhite = int32([currScallop(1) currScallop(2) currScallop(3)]);
