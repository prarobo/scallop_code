function [circleList, imgFilterSteps] = detectTaubinCircle_graph(origImage, bwImage, labelImage, imgFilterSteps )
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    % centroid of edge region
    [rowII, colII] = find( currImage );
    centroid = round([mean(colII) mean(rowII)]);
    centroid(1) = max( 1, centroid(1) );
    centroid(1) = min( size(currImage,2), centroid(1) );
    centroid(2) = max( 1, centroid(2) );
    centroid(2) = min( size(currImage,1), centroid(2) );
    
    centroidReg = labelImage( centroid(2), centroid(1) );
    maxRegArea = sum(labelImage == centroidReg);
    maxReg = centroidReg;
