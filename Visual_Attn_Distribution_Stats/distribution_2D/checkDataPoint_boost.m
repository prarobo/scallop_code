function [ checkData ] = checkDataPoint_boost( currDataPoint, params, percentilePoints, stdDevMaps )
        ...
        for thetaI=1:numThetaBins
            ...
            if(~percentilePoints.(FeatureColor).isValid(radI,thetaI))
                ...
                if(percentilePoints.(FeatureColor).checkMat{radI,thetaI}(currBinNum+1))
                    resultsData.(FeatureColor)(radI,thetaI) = 1/stdDevMaps.(FeatureColor)(radI,thetaI);
                end
            end
            ...
            if(~percentilePoints.(FeatureColorBG).isValid(radI,thetaI))
                ...
                if(percentilePoints.(FeatureColorBG).checkMat{radI,thetaI}(currBinNum+1))
                    resultsData.(FeatureColorBG)(radI,thetaI) = 1/stdDevMaps.(FeatureColorBG)(radI,thetaI);
                end
            end           
        end
    end   
...
for featI = 1:numFeatures
    ...
    for radI = 1:numRadBins
        for thetaI=1:numThetaBins
            ...
            % Accepted Points
            if resultsData.(FeatureColor)(radI,thetaI) > 0 
                checkData.scallopMatchPoints = checkData.scallopMatchPoints +resultsData.(FeatureColor)(radI,thetaI);
            end
            if resultsData.(FeatureColorBG)(radI,thetaI) > 0 
                checkData.bgMatchPoints = checkData.bgMatchPoints + resultsData.(FeatureColorBG)(radI,thetaI);
            end
            if (resultsData.(FeatureColor)(radI,thetaI) > 0 && resultsData.(FeatureColorBG)(radI,thetaI) > 0)
                checkData.bothMatchPoints = checkData.bothMatchPoints ...
                    + resultsData.(FeatureColor)(radI,thetaI)/2 + resultsData.(FeatureColorBG)(radI,thetaI)/2;
            end
