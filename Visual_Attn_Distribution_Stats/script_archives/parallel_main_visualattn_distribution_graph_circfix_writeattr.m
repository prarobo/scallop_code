...
distributionFile = sprintf('%s/scallopDistr.mat', imagesDataDir);
...
if( ~any( strcmp( fieldnames(visualAttnData), 'segmentData' ) ) )
    ...
    visualAttnData ...
        = parallel_segmentObjects_graphcut_circfix( visualAttnData, fixationWindowSize, numSegments, storeProcessData );
    save( visualAttnFile, 'visualAttnData' );
else
...
checkOn = true;
writeOn = true;
visualAttnData = parallel_distr_2D_compute_writeCSV_noadjust(visualAttnData, scallopDistribution, discretOn, checkOn, writeOn);
save( visualAttnFile, 'visualAttnData' );
    ...
    if( ~any( strcmp( fieldnames(visualAttnData.distributionData), 'dataPointCheck' ) ) )
        ...
        checkOn = true;
        writeOn = true;
        visualAttnData = parallel_distr_2D_compute_writeCSV(visualAttnData, scallopDistribution, discretOn, checkOn, writeOn);
        save( visualAttnFile, 'visualAttnData' );
    else
