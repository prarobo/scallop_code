...
visualAttnData = parallel_segmentObjects_graphcut_filter( visualAttnData, fixationWindowSize, numSegments );
save( visualAttnFile, 'visualAttnData' );
else
...
funcTime = tic;
if( ~any( strcmp( fieldnames(visualAttnData), 'graphData' ) ) )

fprintf('Segment objects ...\n');
fixationWindowSize = [200 200];
numSegments = 10;
clearGraphBorders = false;
visualAttnData = parallel_graphcut_learn( visualAttnData );
save( visualAttnFile, 'visualAttnData' );
else
    fprintf('I have it! Segmentation information\n');
end
