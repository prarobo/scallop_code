function displayDetectedScallopsGraph( visualAttnData )
...
for fixI=1:numFixations
    ...
    % Graph
    numObj = length(visualAttnData.distributionData.dataPointCheckGraph{imageI}{fixI});
    
    for objI=1:numObj
        colStart = windowRect{imageI}(fixI,1) - 1 + visualAttnData.segmentData.reducedCircListGraph(imageI).fixation{fixI}(objI,1) ...
                    -visualAttnData.segmentData.reducedCircListGraph(imageI).fixation{fixI}(objI,3);
        rowStart = windowRect{imageI}(fixI,2) - 1 + visualAttnData.segmentData.reducedCircListGraph(imageI).fixation{fixI}(objI,2) ...
                    -visualAttnData.segmentData.reducedCircListGraph(imageI).fixation{fixI}(objI,3);
        dotDia = 2*visualAttnData.segmentData.reducedCircListGraph(imageI).fixation{fixI}(objI,3);
        
        [scallopVerdict, scallopMatchPoints] = checkScallop( visualAttnData.distributionData.dataPointCheckGraph,...
                                                    visualAttnData.classData.threshold, imageI, fixI, objI );
        if scallopVerdict
            if (visualAttnData.statData.categoryGraph(imageI).fixations{fixI}(objI) == 1)
                rectangle('Position',[colStart rowStart dotDia dotDia], 'Curvature',[1 1], ...
                    'LineWidth',2, 'EdgeColor','yellow');
            end
            if (visualAttnData.statData.categoryGraph(imageI).fixations{fixI}(objI) == 0)
                rectangle('Position',[colStart rowStart dotDia dotDia], 'Curvature',[1 1], ...
