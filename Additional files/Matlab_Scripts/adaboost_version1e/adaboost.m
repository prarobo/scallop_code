...
switch(mode)
    case 'train'
        ...
        for t=1:itt
            ...
            % classifiers
            estimateclasstotal=sign(estimateclass*model(t).alpha);
            model(t).error = sum(estimateclasstotal~=dataclass)/length(dataclass);
            model(t).classify = (estimateclasstotal==dataclass);
            model(t).classes = estimateclasstotal;
            estimateclasssum=estimateclasssum +estimateclass*model(t).alpha;
            estimateclasstotal=sign(estimateclasssum);
            model(t).cumerror=sum(estimateclasstotal~=dataclass)/length(dataclass);
            model(t).cumclassify = (estimateclasstotal==dataclass);
            model(t).cumclasses = estimateclasstotal;
            
%             if(model(t).cumerror==0), break; end
        end
